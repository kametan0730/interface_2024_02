...
void icmpv6_input(ipv6_device* v6dev, in6_addr source, in6_addr dstination, void *buffer, size_t len) {
    ...
    LOG_ICMPV6("icmpv6 code=%d, type=%d\n", icmp_pkt->code, icmp_pkt->type);
    ...
    switch(icmp_pkt->type){
        case ICMPV6_TYPE_NEIGHBOR_SOLICIATION:{
            ...
            inet_ntop(AF_INET6, &nspkt->target_addr, target_addr_str, INET6_ADDRSTRLEN);
            ...
            if(memcmp(&nspkt->target_addr, &v6dev->address, 16) == 0){
                ...
                LOG_ICMPV6("option mac address! %s\n", mac_addr_toa(nspkt->opt_mac_addr));
        ...
        case ICMPV6_TYPE_NEIGHBOR_ADVERTISEMENT: {
            ...
            add_nd_table_entry(v6dev->net_dev, napkt->opt_mac_addr, napkt->target_addr);
...
void send_ns_packet(net_device* dev, in6_addr target_addr){
    ...
    icmpv6_na *ns_pkt = reinterpret_cast<icmpv6_na *>(ns_buf->buffer);
