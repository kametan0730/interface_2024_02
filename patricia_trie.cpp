...
void dump_patricia_trie_text(patricia_node *root) {
    ...
    while (!node_queue.empty()) {
        ...
        LOG_TRIE("%s/%d (%d) %d nodes - %s\n", ipv6_str, patricia_trie_get_prefix_len(current_node), current_node->bits_len, patricia_trie_get_distance_from_root(current_node), current_node->is_prefix ? "prefix" : "not prefix");
