 ...
 * 16ビットでバイトオーダーを入れ替える
 */
uint16_t swap_byte_order_16(uint16_t v) { return (v & 0x00ff) << 8 | (v & 0xff00) >> 8; }

/**
 * 32ビットでバイトオーダーを入れ替える
 */
uint32_t swap_byte_order_32(uint32_t v) { return (v & 0x000000ff) << 24 | (v & 0x0000ff00) << 8 | (v & 0x00ff0000) >> 8 | (v & 0xff000000) >> 24; }

#ifdef ENABLE_INTERNAL_HTON_NTOH
...
const char *ip_htoa(uint32_t in) { return ip_ntoa(swap_byte_order_32(in)); }
